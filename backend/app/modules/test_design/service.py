...
class TestDesignService:
    def generate(self, request: GenerateRequest) -> GenerateResponse:
        """Run the B-side test design agents for EP/BVA/DT coverage items."""
    ...
    async def fsm(self, request: FsmRequest) -> FsmResponse:
        """Generate FR4 FSM artifacts through the LLM/prompt pipeline."""
        ...
        try:
            ...
            prompt_evidence = _prompt_records_to_evidence(
                ...
                "FR4 FSM generated by fsm_modeling LLM prompt pipeline.",
            )
...
def _deterministic_fsm_fallback(
    ...
    """Keep FR4 usable when the LLM pipeline is unavailable or returns invalid JSON."""
    ...
    prompt_evidence = [
        evidence(
            ...
            f"FR4 deterministic fallback used because fsm_modeling prompt failed: {failure}",
        )
    ]
        ...
        coverage_items.append(
            {
                ...
                "expected_action": str(item.get("expected_result") or "Exercise FSM transition path."),
                "strategy_rationale": str(item.get("strategy_id") or "FSM state/transition coverage"),
                "technique_reason": "State-related behavior is best covered by FSM state transition testing.",
                "status": "ai_generated",
            }
        )
...
def _fsm_source(request: FsmRequest) -> tuple[str, str, dict[str, Any]]:
    ...
    fallback_text = "The system has stateful behavior that should be modeled with a finite state machine."
...
def _to_test_case(item: dict[str, Any]) -> TestCase:
    ...
    return TestCase(
        ...
        technique="FSM",
        title=str(item.get("title") or "FSM state transition test case"),
        ...
        standard_ref=str(item.get("standard_ref") or "ISTQB state transition testing / finite state machine testing"),
        ...
        normalized.append(
            {
                **item,
                ...
                "expected_action": item.get("description", "Exercise FSM coverage item."),
                "strategy_rationale": str(item.get("strategy") or "FSM coverage strategy"),
                "technique_reason": "State-related behavior is best covered by FSM state transition testing.",
                "status": "ai_generated",
            }
        )
